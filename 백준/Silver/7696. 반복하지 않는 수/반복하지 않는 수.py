def sol(length, num, flag, digits):
    global idx
    if idx > M: return
    if not length: nums[idx] = num; idx += 1; return
    for digit in digits:
        if flag and digit == 0: continue
        digits.remove(digit)
        sol(length - 1, num + digit * (10 ** (length - 1)), False, digits)
        digits.append(digit); digits.sort()

def main():
    ans = []
    for length in range(1, 9): sol(length, 0, True, list(range(10)))
